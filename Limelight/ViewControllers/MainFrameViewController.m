            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                    [self hideLoadingFrame:^{
                        ...
                        if (portTestResult == 0) {
                            message = @"This network does not appear to be blocking TracePort. If you still have trouble connecting, check your PC's firewall settings.\n\nVisit the Moonlight Setup Guide on GitHub for additional setup help and troubleshooting steps.";
                        }
                            ...
                            message = [NSString stringWithFormat:@"Your current network connection seems to be blocking TracePort. Streaming may not work while connected to this network.\n\nThe following network ports were blocked:\n%s", blockedPorts];
                        }
...
- (void) addHostClicked {
    ...
    UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Add Host Manually" message:@"If TracePort doesn't find your computer automatically,\nenter its IP address" preferredStyle:UIAlertControllerStyleAlert];
                ...
                [self->_discMan discoverHost:hostAddress withCallback:^(TemporaryHost* host, NSString* error){
                    if (host != nil) {
                        ...
                        if (portTestResults != ML_TEST_RESULT_INCONCLUSIVE && portTestResults != 0) {
                            error = [error stringByAppendingString:@"\n\nYour device's network connection is blocking TracePort. Streaming may not work while connected to this network."];
                        }
